...
<form action="../shop/cartbuy.jsp">
<h4>${cart} </h4>
<h5 style="text-align: right">상품개수 ${list.size()}</h5>
...
<td>${vo.name}</td>
<td>                   <a href="update.do?cID=${vo.cID}&uID=${vo.uID}&cnt=${vo.cnt-1}">
                  <button  type="button" class="btn btn-light btn-md mr-1 mb-2" onclick="this.parentNode.querySelector('input[type=number]').stepDown()"
                    class="minus">-</button></a></td>
<td> <input class="quantity" min="1" name="cnt" value="${vo.cnt}" type="number" id="amount" style="width: 40px; text-align: center"> </td>
<td>                  <a href="update.do?cID=${vo.cID}&uID=${vo.uID}&cnt=${vo.cnt+1}">
                  <button  type="button" class="btn btnbtn-md mr-1 mb-2" onclick="this.parentNode.querySelector('input[type=number]').stepUp()"
                    class="plus">+</button></a></td>
<td style="text-align: right"><fmt:formatNumber type="number" maxFractionDigits="3" value="${vo.price}"/></td>
<td><a href="delete.do?cID=${vo.cID}&uID=${vo.uID}"><button type="button" class="btn btn-danger">x</button></a></td>
</tr>
</c:forEach>
</table>
<br>
...
<h3 style="text-align: right">총 금액 : <fmt:formatNumber type="number" maxFractionDigits="3" value="${total}"/> <br> </h3>			
					<input type="hidden" name="name" value="${name} 외 ${list.size()} 건" > 			
					<input type="hidden" name="total" value="${total}"> 			

<center><button class="btn btn-success" style="width: 130px; margin: 5px;">주문하기</button></center>
</form>
<br>
<br>
<br>
